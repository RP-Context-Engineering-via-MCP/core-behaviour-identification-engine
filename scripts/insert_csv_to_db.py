...
def main():
    parser = argparse.ArgumentParser(description="Insert behaviors CSV into Supabase with embedding generation.")
    ...
    print("Generating embeddings for all behaviors...")
    texts = df["behavior_text"].fillna("").tolist()
        ...
        try:
            ...
            for t_field in ["created_at", "last_seen_at", "last_decay_applied_at", "last_accessed_at"]:
                if t_field in row and pd.notna(row[t_field]):
                    try:
                        # Assuming bigint ms in CSV, converting to seconds for datetime
                        record[t_field] = datetime.fromtimestamp(float(row[t_field]) / 1000).isoformat()
